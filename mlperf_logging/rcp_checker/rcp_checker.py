...
class RCP_Checker:
    ...
    def _prune_rcps(self):
        ...
        fastest_min_conv = min(min_epochs, key=lambda rc: rc['Min Epochs'])
        fastest_conv = min(min_epochs, key=lambda rc: rc['RCP Mean'])
        min_epochs = list(
            filter(
                lambda rc: (
                    rc["BS"] >= fastest_conv["BS"] or rc["BS"] >= fastest_min_conv["BS"]
                ),
                min_epochs,
            )
        )
        ...
        while i < list_len - 1:
            ...
            if (
                min_epochs[i]["Min Epochs"] > rcp["Min Epochs"]
                and min_epochs[i]["RCP Mean"] > rcp["RCP Mean"]
            ):
                del min_epochs[i]
                i = i-1
                list_len = list_len - 1
